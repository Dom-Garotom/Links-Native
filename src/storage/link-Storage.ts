...
async function remove( removeLink : string){
    try{
        const storage = await get();
        const update = storage.filter( (item) => item.id != removeLink );
        const remove = JSON.stringify(update)
        console.log(remove)

        AsyncStorage.setItem(key_Link_Storage , remove );
